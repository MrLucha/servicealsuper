...
function scrapProducts($link){
    ...
    $buscador = $html->find('mat-label[class=as-roboto-slab as-font-18 as-font-line-through ng-star-inserted]',0);

    //Si hay precio especial
    if ($buscador!=null) {
        $precio=$html->find('mat-label[class=as-roboto-slab as-font-24 as-font-bold as-font-error]');
        foreach ($precio as $p) {
            $p=$p->text();
            $eliminar = array("\t", "$", " ");
    ...
    //Si no hay precio especial
        ...
        $precio=$html->find('mat-label[class=as-roboto-slab as-font-24 as-font-bold]');
        foreach ($precio as $p) {
            $p=$p->text();
            $eliminar = array("\t", "$", " ");
            $precio_producto = str_replace ( $eliminar, '', $p);
